...
void CopyFrame(AVFrame *pFrame, FrameCanvas *canvas) {
  ...
  for(int y = 0; y < height; ++y) {
    ...
    for(int x = 0; x < width; ++x, ++pix) {
      if(y >= 32) {
	      canvas->SetPixel(x+64,y-32,pix->r, pix->g, pix->b);
      }
      else {
	      canvas->SetPixel(64-x, 32-y, pix->r, pix->g, pix->b);
      }
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  RGBMatrix *matrix2 = CreateMatrixFromOptions(matrix_options, runtime_opt);

  FrameCanvas *offscreen_canvas = matrix->CreateFrameCanvas();
  FrameCanvas *offscreen_canvas2 = matrix2->CreateFrameCanvas();
    ...
    while (!interrupt_received && av_read_frame(pFormatCtx, &packet) >= 0
      ...
      if (packet.stream_index==videoStream) {
        ...
        if (frameFinished) {
          ...
          CopyFrame(pFrameRGB, offscreen_canvas2);
          frame_count++;
          frames_left--;
          if (stream_writer) {
            ...
            offscreen_canvas = matrix2->SwapOnVSync(offscreen_canvas2);
          }
        }
        if (!stream_writer) {
